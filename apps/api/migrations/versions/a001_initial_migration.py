...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('route_to', sa.Enum('KITCHEN', 'BAR', name='routedestination'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('FREE', 'OCCUPIED', 'BILL_REQUESTED', name='tablestatus'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('OPEN', 'IN_PROGRESS', 'READY', 'DELIVERED', 'PAID', 'CANCELLED', name='orderstatus'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('order_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('split_type', sa.Enum('BY_SEAT', 'EVEN', 'CUSTOM', name='splittype'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING', 'STAMPED', 'CANCELLED', 'ERROR', name='cfdistatus'), nullable=False),
...
def downgrade() -> None:
    ...
    sa.Enum(name='tablestatus').drop(op.get_bind())
    sa.Enum(name='orderitemstatus').drop(op.get_bind())
    sa.Enum(name='routedestination').drop(op.get_bind())
    sa.Enum(name='cfdistatus').drop(op.get_bind())
    sa.Enum(name='splittype').drop(op.get_bind())
